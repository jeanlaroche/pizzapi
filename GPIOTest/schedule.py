...
def computeGraphData():
    ...
    airData = [line for line in allLines if line[0] == 'A']
    airData = [item.split()[1:3] for item in airData]
    airData = [[float(item[0]),float(item[1])] for item in airData]
    ...
    return heaterTime,heaterValue,usageStr,avUsageStr,thisDayStr,prevDayStr,nextDayStr,stats,airTime,airVal

def logHeaterUse():
    global lastHeaterVal, lastTempVal, fileUpdated, lastTempTime, lastHeaterTime, lastAirTempVal, lastAirTempTime
    ...
    if abs(lastAirTempVal - rt.airTemp) > 0.1 and rt.airTemp:
        if 60*(curTime - lastAirTempTime) > 4:
            statLogF.write("A {:.2f} {} {}\n".format(curTime,rt.airTemp,timeStr))
            lastAirTempVal = rt.airTemp
    else:
        lastAirTempTime = curTime
